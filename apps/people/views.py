...
class PeopleImageView(generic.DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        draw.text((420, 385), self.object.name, (0,0,0), font=font)
        draw.text((420, 490), self.object.identification_id, (0,0,0), font=font)
        draw.text((420, 595), self.object.birthday.strftime("%d-%m-%Y") , (0,0,0), font=font)
        draw.text((420, 700), self.object.nationality, (0,0,0), font=font)
        draw.text((420, 805), self.object.mobile, (0,0,0), font=font)
        collection_time = self.object.collection_time.astimezone(timezone.get_current_timezone())
        draw.text((1670, 490), collection_time.strftime("%Y-%m-%d          %H:%M:%S"), (0,0,0), font=font)
        result_time = self.object.result_time.astimezone(timezone.get_current_timezone())
        draw.text((1670, 595), result_time.strftime("%Y-%m-%d          %H:%M:%S"), (0,0,0), font=font)
        draw.text((1670, 700), self.object.report_no, (0,0,0), font=font)
        draw.text((1670, 805), self.object.hesn_no, (0,0,0), font=font)
