...
class PeopleImageView(generic.DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        draw.text((420, 390), self.object.name, (0,0,0), font=font)
        draw.text((420, 495), self.object.identification_id, (0,0,0), font=font)
        draw.text((420, 600), self.object.birthday.strftime("%d-%m-%Y") , (0,0,0), font=font)
        draw.text((420, 705), self.object.nationality, (0,0,0), font=font)
        draw.text((420, 810), self.object.mobile, (0,0,0), font=font)
        collection_time = self.object.collection_time.astimezone(timezone.get_current_timezone())
        draw.text((1670, 495), collection_time.strftime("%Y-%m-%d          %H:%M:%S"), (0,0,0), font=font)
        result_time = self.object.result_time.astimezone(timezone.get_current_timezone())
        draw.text((1670, 600), result_time.strftime("%Y-%m-%d          %H:%M:%S"), (0,0,0), font=font)
        draw.text((1670, 705), self.object.report_no, (0,0,0), font=font)
        draw.text((1670, 810), self.object.hesn_no, (0,0,0), font=font)
